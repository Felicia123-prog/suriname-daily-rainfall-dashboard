...
st.set_page_config(
    page_title="Maandelijkse Neerslagrapportage 2025–2026",
    layout="wide"
)
...
if mode == "Geheel Suriname (landelijk gemiddelde)":
    ...
    if not has_2025 and not has_2026:
        st.warning(
            "Voor deze maand ontbreken landelijke gegevens voor zowel 2025 als 2026. "
    ...
    if not has_2025:
        st.warning(
            "Voor deze maand ontbreken landelijke gegevens voor 2025. "
            "Een vergelijking met 2026 is niet mogelijk."
        )
        st.stop()

    if not has_2026:
        st.warning(
            "Voor deze maand ontbreken landelijke gegevens voor 2026. "
            "Een vergelijking met 2025 is niet mogelijk."
        )
        st.stop()

    # Dagelijkse gemiddelden
    df26_daily = (
        df_2026_m.groupby("day")["rr"]
        .mean()
        .round(1)
        .reset_index()
    )
    df25_daily = (
        df_2025_m.groupby("day")["rr"]
        .mean()
        .round(1)
        .reset_index()
    )
    ...
    st.info(
        f"📡 Beschikbare stations — 2026: **{stations_2026}** | "
        f"2025: **{stations_2025}**"
    )
    ...
    st.markdown(
        ...
        f"De regenval was redelijk gelijkmatig verdeeld, met een hoogste gemiddelde dagwaarde "
        f"van **{max_avg26:.1f} mm**.\n\n"
...
# ---------------------------------------------------------
    ...
    if not has_2025 and not has_2026:
        st.warning(
            f"Voor station **{station}** ontbreken gegevens voor zowel 2025 als 2026. "
            "Een vergelijking is niet mogelijk."
        )
        st.stop()
    ...
    if not has_2025:
        st.warning(
            f"Voor station **{station}** ontbreken gegevens voor 2025. "
            "Een vergelijking met 2026 is niet mogelijk."
        )
        st.stop()
    ...
    if not has_2026:
        st.warning(
            f"Voor station **{station}** ontbreken gegevens voor 2026. "
            "Een vergelijking met 2025 is niet mogelijk."
        )
        st.stop()

    df26_s["label"] = df26_s.apply(
        lambda r: "Cumulatief" if r["is_cumulative"] else "Dagwaarde",
        axis=1
    )
    df25_s["label"] = df25_s.apply(
        lambda r: "Cumulatief" if r["is_cumulative"] else "Dagwaarde",
        axis=1
    )
    ...
    day_max26 = (
        df26_s.loc[df26_s["rr_value"] == max26, "day"].iloc[0]
        if max26 > 0 else "-"
    )
    day_max25 = (
        df25_s.loc[df25_s["rr_value"] == max25, "day"].iloc[0]
        if max25 > 0 else "-"
    )
...
- **Totale neerslag:** {wetter_year} had **{abs(total26-total25):.1f} mm** meer neerslag.
- **Gemiddelde dagneerslag:** verschil van **{abs(avg26-avg25):.1f} mm/dag**.
- **Hoogste dagwaarde:** verschil van **{abs(max26-max25):.1f} mm**.
